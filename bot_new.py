...
import notion_cluster
# Re-export Notion symbols used by non-Notion callers (family_lookup, startup health check, etc.)
from notion_cluster import (
    NOTION_API, NOTION_HEADERS,
    _notion_fetch_children, notion_read_page, notion_append_bullet,
)
...
TOOLS = [
    ...
    *notion_cluster.SCHEMAS,
    ...
    {"name":"icloud_calendar","description":"Get upcoming events from Sean's iCloud Calendar. Default window is the next 30 days; pass days=N to look further ahead (e.g. days=180 for a trip a few months out). If Sean asks about an event that is not appearing, widen the window with days before concluding it is missing. Each event line is formatted as `- YYYY-MM-DD HH:MM: summary  [cal: <calendar_name> | uid: <event_uid>]` -- pass the uid and cal name to icloud_calendar_delete or icloud_calendar_move to chain operations.","input_schema":{"type":"object","properties":{"max_results":{"type":"integer","default":10},"days":{"type":"integer","default":30,"description":"How many days ahead to search. Default 30. Increase for events further out."}}}},
    ...
    {"name": "imessage_search", "description": "Search Sean's iMessage history for messages whose text contains the query (substring match). Use for 'when did Heather mention X', 'find that text from Sudhir about Y'. Searches the last 168 hours (7 days) by default; pass hours= for a wider window. Text-only search — does not match images or attachments. If results are empty AND the query looks like a partial phone number (3+ digits like '484', '215'), do NOT speculate about possible reasons -- call imessage_handles_search next to find the full handle, then retry. If results are empty for non-numeric queries, be honest rather than fabricating.", "input_schema": {"type": "object", "properties": {"query": {"type": "string"}, "max_results": {"type": "integer", "default": 20}, "hours": {"type": "integer", "default": 168}}, "required": ["query"]}},
    ...
    {"name": "imessage_thread", "description": "Read Sean's iMessage CONVERSATION with a SPECIFIC PERSON, including messages he has ALREADY READ, both directions (sent + received), newest first. Use when Sean says \"what did <person> say\", \"read my texts with <person>\", \"pull up my conversation with <person>\", \"show me <person>'s messages\" \u2014 especially for already-read messages that imessage_unread (unread-only) misses. The person can be a NAME (resolved via Sean's Contacts + family whitelist, e.g. \"Lindsey\", \"Heather\") or a raw phone/email handle. NOT limited to the family send-whitelist \u2014 can read any conversation in Messages. If a name matches multiple contacts the bridge merges their handles. If the input is a PARTIAL phone number (e.g. '484' rather than a full +1XXXXXXXXXX), resolution will fail with 0 results -- call imessage_handles_search FIRST to find the full handle, then retry imessage_thread with that full handle. hours=0 (default) searches full history capped by max_results; set hours>0 to limit to a recent window.", "input_schema": {"type": "object", "properties": {"name_or_handle": {"type": "string", "description": "Person's name (e.g. 'Lindsey') or a phone number / email address."}, "max_results": {"type": "integer", "default": 20, "description": "Max messages to return (1-200, newest first)."}, "hours": {"type": "integer", "default": 0, "description": "0 = full history (default); >0 limits to last N hours."}}, "required": ["name_or_handle"]}},
    {"name": "imessage_handles_search", "description": "Search Sean's iMessage handle list for any address (phone/email) containing the query substring. Returns matching handles with msg_count + last_seen, sorted by recency. USE THIS FIRST whenever Sean asks about a partial phone number (e.g. 'messages from the 484 number', 'that 215 contact', 'what did the 410 guy text me') -- imessage_search and imessage_thread will return 0 for these queries because they match contact names + full handles + message body text, NOT partial digit substrings. Pick the right handle from the result, then call imessage_thread with the full handle. Per Shape 2: if you called imessage_search/imessage_thread first for a partial-number query and got 0, the fix is to call THIS tool -- do not speculate about why ('might be SMS only', 'older than 7 days', etc.) without checking here first.", "input_schema": {"type": "object", "properties": {"query": {"type": "string", "description": "Substring to search for in handles (min 3 chars). Digits work for area codes; can also match email substrings."}, "max_results": {"type": "integer", "description": "Max matching handles to return (1-100, default 20). Results sorted by last contact, newest first."}}, "required": ["query"]}},
...
async def run_tool(name, inputs):
    ...
    _modular_dispatch = {**security_recon.DISPATCH, **memory_history.DISPATCH, **notion_cluster.DISPATCH}
    ...
    elif name=="imessage_handles_search":
        _q = (inputs.get("query") or "").strip()
        if not _q: return "ERROR: imessage_handles_search requires query."
        try: _max = int(inputs.get("max_results", 20))
        except: _max = 20
        return await asyncio.to_thread(imessage_handles_search, _q, _max)
...
Finance-commit push-back: When Sean explicitly asks to save finance, holdings, stock-order, insurance, or mortgage data to CORE memory (e.g., 'commit that to your core memory' after discussing a position or policy), DO NOT silently comply. First respond by surfacing the architectural concern: e.g., 'We keep finance data out of core memory because price-sensitive snapshots go stale fast — the canonical source for stock positions is Finance/Investments/Portfolio.xlsx on family Drive. Want me to save this as a reference-tier note instead, or add a row to the sheet?' Wait for explicit confirmation before writing to core. If Sean still wants core after the push-back, comply — but the push-back protects against silent regressions of the document-not-memory pattern. The Portfolio.xlsx sheet ID is 13sGX8Q_8d0DkOt6eHlI-YPdL9vEByfQ7EnDogEbUkVk — you can read it for current holdings.
...
- LIST ROUTING:
  - Default: "To Do List" — use for everything that is not obviously food/household supplies and where Sean hasn't named a specific list.
  ...
  - Other lists: Sean may have additional lists in Reminders.app (e.g. project-specific, family). If he names one explicitly — "add this to my White Feather list", "put it on my Garage list" — pass that name through as list_name. The bridge will accept any list that exists on his Mac. If the bridge returns "unknown list_name" with an "allowed" set, surface it to Sean so he can correct.
...
async def cmd_briefing(update, context):
    ...
    try:
        ...
        text = await build_briefing(
            ...
            get_conn=get_conn,
            notion_query_db_fn=notion_cluster.notion_raw_query_database,
        )
...
def main():
    ...
    start_briefing_scheduler(app,OWNER_TELEGRAM_ID,gmail_get_unread,calendar_get_upcoming,brave_search,check_important_emails,get_conn=get_conn,notion_query_db_fn=notion_cluster.notion_raw_query_database)
...
def icloud_calendar_upcoming(max_results=10, days=30):
    try:
        ...
        for cal in calendars:
            # Resolve calendar display name (fall back to URL slug)
            try:
                _cal_name = (cal.get_display_name() or "").strip()
            except Exception:
                _cal_name = ""
            if not _cal_name:
                try:
                    _cal_name = str(cal.url).rstrip('/').split('/')[-1] or 'default'
                except Exception:
                    _cal_name = 'default'
            try:
                for event in cal.date_search(start=now, end=end, expand=True):
                    ...
                    # UID needed for icloud_calendar_delete / icloud_calendar_move
                    _uid_attr = getattr(vevent, 'uid', None)
                    _uid = str(_uid_attr.value) if _uid_attr is not None and hasattr(_uid_attr, 'value') else '(no-uid)'
                    events.append(f"- {start_str}: {summary}  [cal: {_cal_name} | uid: {_uid}]")
            except: pass
...
def reminders_add(title, list_name="To Do List", due_date=None, notes=None):
    ...
    # No client-side list validation: the bridge holds the authoritative set via
    # AppleScript-backed dynamic lookup (refreshed every ~60s on the Mac side).
    # The bridge returns a 400 with the actual valid list set if rejected; we surface
    # that downstream so Sean gets the real list of options.
...
def imessage_handles_search(query, max_results=20):
    """Search Sean's iMessage handle list for substring matches via Mac bridge /handles_search."""
    import requests as _rq
    _url = os.environ.get("CLAWDIA_IMESSAGE_URL", "")
    _tok = os.environ.get("CLAWDIA_IMESSAGE_TOKEN", "")
    if not _url or not _tok:
        return "imessage_handles_search: CLAWDIA_IMESSAGE_URL or CLAWDIA_IMESSAGE_TOKEN not set in /etc/clawdia/env"
    q = (query or "").strip()
    if not q:
        return "imessage_handles_search: need a query (e.g. '484' for area-code search)."
    try:
        r = _rq.post(_url + "/handles_search",
                     json={"query": q, "max_results": int(max_results)},
                     headers={"X-Clawdia-Token": _tok, "Content-Type": "application/json"},
                     timeout=10)
    except _rq.exceptions.RequestException as e:
        return f"imessage_handles_search BRIDGE_UNREACHABLE: {type(e).__name__}: {str(e)[:200]}"
    if r.status_code != 200:
        try: err = r.json().get("error", r.text[:200])
        except: err = r.text[:200]
        return f"imessage_handles_search error ({r.status_code}): {err}"
    try:
        data = r.json()
    except ValueError:
        return f"imessage_handles_search: non-JSON response: {r.text[:200]}"
    matches = data.get("matches", [])
    if not matches:
        return f"imessage_handles_search: no handles match {q!r}. The substring isn't in any handle (phone/email) in Sean's iMessage DB. {data.get('note', '')}".strip()
    lines = [f"Found {len(matches)} handle(s) matching {q!r} (sorted by last contact):"]
    for m in matches:
        lines.append(f"  - {m.get('handle','?')} | {m.get('msg_count',0)} msgs | last_seen {m.get('last_seen','unknown')}")
    return "\n".join(lines)
